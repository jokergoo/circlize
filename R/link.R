...
chordDiagram.symmetric = function(mat, link.colFun = NULL) {
    n = nrow(mat)
    xlim = cbind(rep(0, n), rowSums(abs(mat)) - diag(abs(mat)))

    factors = rownames(mat)
    if(is.null(factors)) {
        factors = paste0("V", seq_len(nrow(mat)))
        factors = factor(factors, levels = factors)
    }
    colors = 1:n

    if(is.null(link.colFun)) {
        f = colorRamp2(quantile(mat[lower.tri(mat)], seq(0, 1, length = 7)), rev(rainbow(7)))
        link.colFun = function(x) paste0(f(x), "80")
    }
    ...
    for(i in 2:n) {
        for(j in 1:(i-1)) {
            ...
            circos.link(sector.index1, c(sector.sum[i],sector.sum[i] + abs(mat[i, j])),
                ...
                col = link.colFun(mat[i, j]), border = NA)
...
chordDiagram.bicategorical = function(mat) {
