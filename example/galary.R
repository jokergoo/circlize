...
circos.trackPlotRegion(ylim = c(0, 1), bg.border = NA, track.height = 0.1, panel.fun = function(x, y) {
	i = get.cell.meta.data("sector.numeric.index")
	circos.updatePlotRegion(bg.col = ifelse(i %% 2, "#D4D4D4", "#999999"), bg.border = NA)
	
	xlim = get.cell.meta.data("xlim")
	xrange = get.cell.meta.data("xrange")
	x = seq(xlim[1], xlim[2], length.out = 10)
	y = runif(length(x))
	circos.lines(x, y, area = TRUE, border = NA, col = "#FF7F00")
})
