...
through the whole track. Finally, draw two texts in a certain cell.
...
\section{Axis}
Because there may be no space to draw y-axis, only x-axis for each cell is supported, as illustrated in figure 8.
...
\section{Links}
Links can be drawn from points and intervals (figure 9). If both ends are points, then the link is represented
...
the red line (figure 10). Links do not hold any track.
...
distribution of data (figure 14). So users would know how to implement those
high level function accroding to the source code of {\tt circos.trackHist}.
...
\section{Drawing outside the potting region}
...
> factors = letters[1:4]
> circos.par("canvas.xlim" = c(-2, 2), "canvas.ylim" = c(-2, 2))
> circos.initialize(facotrs = factors, xlim = c(0, 1))
> circos.trackPlotRegion(ylim = c(0, 1), panel.fun = function(x, y) {
+     circos.points(1:20/20, 1:20/20)
+ })
> circos.lines(c(1/20, 0.5), c(1/20,1.5), sector.index = "d")
> circos.text(0.5, 1.5, "mark", sector.index = "d", adj = c(0.5, 0))
>
> circos.trackPlotRegion(ylim = c(0, 1), panel.fun = function(x, y) {
+     circos.points(1:20/20, 1:20/20)
+ })
> circos.rect(-0.1, -0.1, 1.1, 2.1, col = "#FF0000D0")
>
> circos.clear()
\end{Sinput}
\end{Schunk}
