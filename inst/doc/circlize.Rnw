...
Following is an example. First generate some data.
...
draw the figures (figure \ref{transformation}).

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{transformation.pdf}
\caption{coordinate transformation}\label{transformation}
\end{center}
\end{figure}

\section{Sectors and tracks}
A circos layout is composed of sectors and tracks, as illustrated in figure \ref{coordinate}.
The red circle is the track and the blue one is the sector. The intersection of a sector and 
a track is called a cell.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{coordinate.pdf}
\caption{coordinate}\label{coordinate}
\end{center}
\end{figure}

\section{Circos parameters}
A cell has margins and paddings, like in CSS. refers to figure \ref{region}. The directions
of cells in a track are illustrated in figure \ref{direction}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{region.pdf}
\caption{Regions for a cell}\label{region}
\end{center}
\end{figure}
...
\caption{Sector direction}\label{direction}
\end{center}
\end{figure}

\section{Lines}
Parameters for drawing lines by {\tt circos.lines} are similar to {\tt lines} function, 
as illustrated in figure \ref{line}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{line.pdf}
\caption{Lines}\label{line}
\end{center}
\end{figure}

\sector{Text}
Only the direction of text by {\tt circos.text} should be noted, as illustrated in figure \ref{text}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{text.pdf}
\caption{Texts}\label{text}
\end{center}
\end{figure}

\section{Axis}
Only x-axis for each cell, as illustrated in figure \ref{axis}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{axis.pdf}
\caption{axis}\label{axis}
\end{center}
\end{figure}

\section{Links}
Links can be drawn from points and intervals. figure \ref{link}. The height of the link can be controlled
by {\tt top.ratio} argument in {\tt circos.link} which is the ratio between the length of red line and 
the blue line (figure \ref{quadratic_curve}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{link.pdf}
\caption{link}\label{link}
\end{center}
\end{figure}
...
\caption{quadratic_curve}\label{quadratic_curve}
\end{center}
\end{figure}
