...
Sectors are first allocated is determined by {\tt circos.initialize} and track allocation is then determined by {circos.trackPlotRegion}.
...
Order of the sectors are determined by {\bf the order of levels of the factor}. So if you want to change the order
...
has the same number of rows as the length of {\tt factors} levels. Here, every row in {\tt xlim} correspongs to the x-ranges
...
\subsection{The {\tt panel.fun} argument}
...
\subsector{Drawing outside and combine with canvas coordinate}
