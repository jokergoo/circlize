...
Currently there are following functions that can be used for plotting:
...
(or the order of using the circos functions) for drawing. Finally a detailed explaination
of circos parameters, coordinates and basic graphs would be represented. 
...
the unit circle according to ranges of x-values in different categories. E.g,
...
those low level functions can be applied. Here we should specify the y-values either by {\tt y}
or {\tt ylim} argument. The axis for each cell is drawn by {\tt panel.fun} argument. The {\tt panel.fun}
is actually executed after the creation of the plotting region for a cell. After that, draw points
...
Here what should be note is that the first track has a index number of 1 since this is the first track.
...
So if {\tt track.index} is not specifed, it means the track index is the current track index.
...
can also create a new track. So the track index for this track is 2.
...
Draw the third track. Different background colors for cells can be assigned. Here some
meta data for a cell can be obtained by {\tt get.cell.meta.data}. This function needs
...
The function erases graphs which are drawn. Here we erase graphs in one cell in track 2 
...
The rules for drawing the circos layout is rather simple.

\begin{enumerate}
  \item Initialize the layout using {\tt circos.initialize}. There should be a factor and a x-range to allocate 
        sectors.
  \item Create plotting regions for the new track and apply plottings. The new track is created just below
        the previous created one and the index of the track is added by 1. Only after the creation of the track can you
		add other graphs. There are three ways to do the plotting job.
		\begin{enumerate}
			...
			\item Use {\tt panel.fun} argument in {\tt circos.trackPlotRegion} to draw
				  ...
				  two arguments which are {\tt x} and {\tt y} which are x-values and y-values that
...
\begin{itemize}
	...
	\item {\tt vertical\_left}: direction of radius, facing left at $90^\circ$.
	\item {\tt vertical\_left}: direction of radius, facing right at $90^\circ$.
...
\section{Do not forget {\tt circos.clear}}
