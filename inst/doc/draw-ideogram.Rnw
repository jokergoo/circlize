...
and we also do not need the line width to be too wide because genomic graph is a huge graph.
...
chr3, ...'' or else the order would be the character order which is ``chr1, chr11, ...'' (this is the default behavior for the function of {\tt factor}).
...
for each chromosome, 1. assign different colors for different locus, 2. draw rectangle for different locus, 3. add axis, 4. add
chromosome names. Here the color panel is from \url{http://circos.ca/tutorials/course/slides/session-2.pdf}, page 42.
...
\begin{figure}
\begin{center}
<<label=figideogram, fig=TRUE, echo=FALSE, eval=TRUE>>=
<<figideogram3>>
<<figideogram4>>
<<figideogram5>>
<<figideogram6>>
...
<<label=figgenomic, echo=FALSE, eval=FALSE>>=
library(circlize)
set.seed(12345)
...
\begin{figure}
\begin{center}
<<label=figgenomic, fig = TRUE, echo = FALSE, eval=TRUE>>=
<<figgenomic>>
@
\end{center}
...
<<label=figtwo, echo=FALSE, eval=FALSE>>=
...
\begin{figure}
\begin{center}
<<label=figtwo, fig = TRUE, echo = FALSE, eval=TRUE>>=
<<figtwo>>
@
\end{center}
