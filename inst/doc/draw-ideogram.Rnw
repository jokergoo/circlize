...
In the data frame, the second column and the third column are intervals for locus' intervals.
...
Since chromosomes are sorted by their names which are as mode of character, the order would look
...
Second way, draw ideogram directly as the creation of the plotting region by using {\tt panel.fun} argument.
...
a new track. Here there no specifying of {\tt factors}, thus the new track would create plotting regions for all available
sectors.
...
circos layout is really subjective (such the position and colors of the ideogram, or maybe only subset of chromosomes
are going to be plotted), users can draw their style of ideogram according
the above example codes.

Finally, more beautiful genomic graphs are figure 2 and figure 3.
