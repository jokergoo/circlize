...
In most circumstances, the figures contain an ideogram. This document
...
In the data frame, the second column and the third column are intervals for locus.
...
Calculate the range for each chromosome which is going to be the {\tt xlim} of
each sector. In the following code, order of rows of {\tt xlim} correspond to the 
order of elements in {\tt chromosome}.
...
Set some graphic parameters. Here we do not need any cell paddings.
...
is not set, {\circos.trackPlotRegion} will create plotting regions for all available sectors
which have already been initialized automatically.
...
circos layout is really subjective, such the position and colors of the ideogram, 
or maybe only subset of chromosomes are going to be plotted, or maybe there are some zoomings for certain chromosomes
(see \url{http://circos.ca/intro/features/}. `GLOBAL AND LOCAL ZOOMING' section). Users can draw their style of ideogram according
the above example codes. All you need to remember is that complicated graphs are assembled by simple graphs.

Finally, more informative and specialized genomic graphs are figure 2 and figure 3.
...
circos.trackPlotRegion(ylim = c(-1, 1), bg.border = NA, bg.col ="#EEEEEE", track.height = 0.1, panel.fun = function(x, y) {
	...
	for(i in -2:2) {
		circos.lines(xlim, c(i, i)/4, col = "white", lwd = 0.5, lty = 2)
	}
	xrange = get.cell.meta.data("xrange")
	x = NULL
	y = NULL
...
xlim.chr1 = get.cell.meta.data("xlim", sector.index = "chr1", track.index = 1)
circos.rect(xlim.chr1[1], -5.5, xlim.chr1[2], 2, col = "#FF000020", border = NA, sector.index = "chr1", track.index = 1)
