...
The characteristic of the circos layout is the link (or connector) to represent relations
...
Then the range for each element, which is the summation of the correlations with other elements
...
Initialize the layout and add the name for elements around each sectors.
...
The finnal figure looks like figure \ref{correlation}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{correlation.pdf}
\caption{Representing correlation matrix by circos layout}\label{correlation}
\end{center}
\end{figure}
