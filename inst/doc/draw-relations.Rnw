...
between elements (\url{http://circos.ca/intro/tabular_visualization/}). 
Here we show how to represent a correlation matrix.
...
Initialize the layout and add names for elements around each sector.
