...
\arguments{
  ...
  \item{f1}{a self-defined function with no argument for making the first circular plot}
  \item{f2}{a self-defined function with no argument for making the second circular plot}
  \item{correspondance}{a six-column data frame which contains correspondance between the coordinates in two circular plots}
  \item{connection_height}{the height of the connections, measured as the percent to the radius of the unit circle. The value can be specified by \code{\link{uh}} or \code{\link{convert_height}} with absolute units.}
  \item{connection_col}{filled color for the connection polygons. The value can be a vector with same length as number of rows of \code{correspondance}}
  \item{connection_border}{border color for the connection polygons}
  \item{connection_lty}{line style for the connection polygon borders}
  \item{connection_lwd}{connection polygon border width}
  \item{adjust_start_degree}{If \code{start.degree} is not set in \code{f2()}, the start degree for the second circular plot will be adjusted to make the distance of sectors between the two plots to the minimal.}
