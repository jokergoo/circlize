...
\describe{
  \item{start.degree}{The starting degree which the circle begin to draw. Note this degree is measured in the standard polar coordinate which means it is anti-clockwise.}
  \item{gap.degree}{Gap between two neighbour sectors. Note there is a gap in front of the first sector.}
  \item{track.margin}{Like \code{margin} in Cascading Style Sheets (CSS), it is the blank area out of the plotting region, also outside of the borders. Since left and right margin are controlled by \code{gap.degree}, only bottom and top margin need to be set. The value for the \code{track.margin} is the percentage according to the radius of the unit circle.}
  \item{unit.circoe.segments}{Since curves are simulated by a series of straight lines, this parameter controls the amout of segments to represent a curve. The minimal length of the line segmentation is the length of the unit circle / \code{unit.circoe.segments}.}
  ...
  \item{default.track.height}{The default height of tracks. It is the percentage according to the radius of the unit circle. The height includes the top and bottom cell paddings but not the margins. However, the definition would be changed in future version because I think it would be more  reasonable to include the margins in the track.}
  \item{points.overflow.warning}{Since each cell is in fact not a plotting region but only an ordinary rectangle, it does not eliminate points that are plotted out of the region. So if some points are out of the plotting region, by default, the  package would continue drawing the points and print warnings. But in some  circumstances, draw something out of the plotting region is useful, such as draw some legend or text. Set this value to \code{FALSE} to turn off the warnings.}
  \item{canvas.xlim}{The coordinate for the canvas. By default, the package draws unit circle, so the xlim for the canvas would be \code{c(-1, 1)}. However, you can set it to a more broad interval if you want to draw other things out of the circle. By choose proper \code{canvas.xlim} and \code{canvas.ylim}, you can draw part of the circle. E.g. setting \code{canvas.xlim} to \code{c(0, 1)} and \code{canvas.ylim} to \code{c(0, 1)} would only draw circle in the region of (0, pi/2).}
  \item{canvas.ylim}{The coordinate for the canvas. By default it is \code{c(-1, 1)}}
  \item{clock.wise}{The order of drawing sectors. Default is \code{TRUE}.}
}
