...
In the second track, we are going to plot the circular dendrogram. Here we implement the code as {\tt circos.dendrogram} in a recursive way. The
...
\caption{A simple phylogenetic tree. Top: circular style; Bottom: normal style.}
