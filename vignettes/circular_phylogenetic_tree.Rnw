...
render the dendrogram by \textbf{dendextend} package (\ref{fig:phylogenetic_tree} A).
...
adjust the name track to the inside of the dendrogram track ((\ref{fig:phylogenetic_tree} B).
