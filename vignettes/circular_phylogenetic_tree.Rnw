...
In this short vignette, we will show you how to customize the circular style of 
phylogenetic tree through {\it circlize} package. Basically, a phylogenetic tree is 
...
The {\tt bird.orders} data we are using here is from {\tt ape} package. This data set is 
related to species of birds. Some other values such as labels for birds (birds' names) 
are also extracted. We split the tree into six sub trees by {\tt cutree}
...
Maximum height of tree {\tt maxy} here is not necessary, but it would be important if there
are more than one tress that are put on the same track.
...
execution will stop. You can see the advantage of {\it circlize} that if you replace 
{\tt circos.lines} to {\tt lines} in the function, then you can almost
...
In figure \ref{fig:phylogenetic_tree} (top), we additionally cut the dendrogram into six 
