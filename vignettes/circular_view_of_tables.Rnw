...
{\tt grid.col} or {\tt circos.par("gap.degree")} will also be reduced if it is set as a vector
which has the same length as categories which are from the unreduced matrix. (figure \ref{fig:chordmisc}, bottom)
...
@
\end{center}
\caption{Reduced Chord Diagram with removing tiny categories. Topright: notice how link colors are reduced; bottom left: notice how grid color are reduced; bottom right notice how gap degrees are reduced.}
\label{fig:chordmisc}
\end{figure}
\setkeys{Gin}{width=0.8\textwidth}
