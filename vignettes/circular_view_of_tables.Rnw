...
\subsection{Compare two Chord Diagram}

Normally, values in {\tt mat} are normalized to the summation, which means the width for each link only
represents kind of relative value. 

First, plot a Chord Diagram:
...
Now we calculate the blank area

<<echo=TRUE, eval=FALSE>>=
percent = sum(mat2) / sum(mat1)
blank_degree = (360 - 2*8) * (1 - percent)
@

plot the second Chord Diagram

<<echo=TRUE, eval=FALSE>>=

gap.degree = c(rep(2, 4), blank_degree/2, rep(2, 4), blank_degree/2)
circos.par(gap.degree = gap.degree, start.degree = -blank_degree/4)
