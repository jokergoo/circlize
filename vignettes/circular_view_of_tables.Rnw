...
\caption{Basic settings for plotting Chord diagram. Top left: default style; Top right: set {\tt gap.degree};
         Middle left: set {start.degree}; Middle right: set orders of sectors; Bottom left: set {\tt directional};
...
use {\tt circos.clear} to reset it.
...
The order of sectors can be controlled by {\tt order} argument (figure \ref{fig:chordbasic}, middle right) although it seems not necessary.
...
If {\tt grid.col} has no name index, the order just corresponds to the order of sector names.
...
to customize the Chord diagram with different styles. In figure \ref{fig:chordadvanced} (bottom left),
...
@
\end{center}
\caption{Chord diagram for other matrix. Top left: with directions; Top right: symmetric correlation matrix; Top bottom: without self-loop}
...
is set to {\tt TRUE}, only lower triangular matrix without the diagonal will be used (figure \ref{fig:chordother}, bottom. Of course, your matrix should be symmetric).
...
unit width of links should represent same scale. This problem can be solved by add more blank gaps to the Chord Diagram which has smaller summation.

First, let's plot a Chord Diagram. In this Chord Diagram, we set larger gap between rows
...
@
\end{center}
\caption{Compare two Chord Diagrams and make them same scale}
