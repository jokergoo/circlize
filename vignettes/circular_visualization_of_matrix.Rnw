...
In {\tt circlize}, it is easy to plot it in a straightforward or customized way.
...
is simply the summation in rows or columns respectively.
...
Above codes show the basic idea to implement Chord Diagram in {\it circlize}. Of course
you can customize the plot in your own way such as changing the order on ending states 1 to 6
...
that why we add {\tt set.seed} to make sure that users can make same figures with the same code.
The link colors are same as colors for grids which correspond to rows. The order of 
sectors is the order of {\tt union(rownames(mat), colnames(mat))}.

Since Chord Diagram is implemented by {\it circlize} package, like normal circos plot,
...
as colors for grids which correspond to rows (figure \ref{fig:chord_diagram_color} A).
...
In the following example, we use {\tt rand\_color} which is shippped by {\it circlize} package
...
<<chord_diagram_color, echo = FALSE, fig.align = "center", out.width = "\\textwidth", out.height = "1.5\\textwidth", fig.width = 7, fig.height = 10.5, fig.cap = "Color settings in {\\tt chordDiagram}. A) set {\\tt grid.col}; B) set {\\tt transparency}; C) set {\\tt col} as a matrix; D) set {\\tt col} as a function; E) set {\\tt row.col}; F) set {\\tt column.col}">>=
...
If it is set as a matrix, it should corresponds to rows and columns in {\tt mat} 
...
There are at least two ways to do it. First, by constructing a color matrix.
...
Second, use a color function to generate different colors with differnet values.
...
Remember if you want to highlight links by colors, make sure to set high or full
...
through {\tt diffHeight} (figure \ref{chord_diagram_directional} B, C). 
In {\tt chordDiagram}, the starting foot of the link is shorter
than the ending foot to give people the feeling that the link is pointing out.
...
<<>>=
for(cn in intersect(rownames(mat2), colnames(mat2))) {
    mat2[cn, cn] = 0
}
mat2
@
...
you may see the difference between specifying {\tt symmetric} or not when visualizing a
symmetric matrix. 
...
By default, {\tt chordDiagram} which utilizes circos graphics function will create two tracks,
...
but with {\tt preAllocateTracks} it is rather easy to customize them which we will introduce
in next section.
...
of {\tt major.tick.percentage} is the ratio to the height of the track where the axes are plotted.
...
And the axes are only added if the width of the sector is larger than 20 degree (figure \ref{fig:chord_diagram_axes} B).
...
Normally, values in {\tt mat} are normalized to the summation, and each value is put to 
...
In the following matrix, the second row and third column contains tiny numbers.
