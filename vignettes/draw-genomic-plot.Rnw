...
\begin{figure}[h!]
\centering
\includegraphics{figures/circos_genomictrackplotregion.pdf}
\caption{Plotting points in differnet ways. Topleft: Plotting points from A) a data frame with one numeric column;
...
four numeric column and under {\tt stack} mode. Topright: Plotting lines in differnet ways. Plotting lines from A) a data frame with one numeric column;
...
For example, some horizontal lines and texts need to be added to each cell and axis need to put on top of each cell:
