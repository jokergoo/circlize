...
The first column is genomic category (e.g. chromosome), the second column is the start position
...
{\it circlize} provides a simple function {\tt colorRamp2} which wraps {\tt colorRamp}
to interpolate colors. The arguments of {\tt colorRamp2} are break points and colors
which correspond to the the break points. {\tt colorRamp2} returnes a new function
...
All low-level genomic graphical functions will be drawn on the 'horizontal line' {\tt y = i} in which {\tt i} is the index of current numeric column 
