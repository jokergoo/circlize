...
\begin{figure}[h!]
\centering
\includegraphics{figures/circos.initializeWithIdeogram_example.pdf}
...
is applied in every chromosome but with different form.

<<echo=TRUE, eval=FALSE>>=
circos.genomicTrackPlotRegion(data, panel.fun(region, value, ...) {
...
<<echo=TRUE, eval=FALSE>>=
circos.genomicTrackPlotRegion(data, ylim = c(0, 1), panel.fun(region, value, ...) {
	circos.genomicPoints(region, value, ...)
})
circos.genomicTrackPlotRegion(data, numeric.column, panel.fun(region, value, ...) {
	circos.genomicPoints(region, value, ...)
})
@
...
expects an data frame containing regions and a data frame containing values. The data column for 
...
and the graphical parameters should be either length one or number of numeric columns specified (see figure \ref{fig:genomicPoints}, track A and track E).
...
For {\tt lty}, we additional provide a new option {\tt segment} by which each genomic
interval will represent as a 'horizontal' line at corresponding value in {\value} (see figure \ref{fig:genomicLines}, track F).
...
For examples, please see figure \ref{fig:genomicRect}
...
The behavior of {\tt circos.genomicTrackPlotRegion} and {\tt panel.fun} inside 
will be different according to different input data and different settings.

If {\tt data} is a simple data frame, {\tt region} in {\tt panel.fun} would be a data frame
containing start position and end position in the current chromosome which is extracted
from {\tt data}. {\tt value} is also a data frame which contains columns in {\tt data}
...
If {\tt data} is a list of data frames, 
...
to use transparent filled colors.
...
We specifically implement several high-level functions which may help to visualiza genomic data.
...
circos.genomicPosTransformLines(data, posTransform = posTransform.default, horizontalLine = "top")
circos.genomicPosTransformLines(data, posTransform = posTransform.default, type = "reverse")
@
...
circos.genomicDensity(bed_list, col = c("red", "green"))
@
