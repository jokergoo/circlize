...
especially provides functions which focus on genomic plots. These functions 
...
in the genomic category and the third column is the end position. Following columns are optional where
...
generated from human geonme. In the function, {\tt nr} and {\tt nc} are number of rows and numeric columns that users want. Please note {\tt nr}
...
by specifying the path of your cytoband file or providing your cytoband data as a data frame.

<<echo=TRUE, eval=FALSE>>=
cytoband.file = paste(system.file(package = "circlize"),
    "/extdata/cytoBand.txt", sep="")
circos.initializeWithIdeogram(cytoband.file)
cytoband = read.table(cytoband, colClasses = c("character", "numeric",
...
are genomic axis and chromosome names, and create another track where there is an ideogram.
...
\begin{figure}[h!]
\centering
\includegraphics{figures/circos_initializeWithIdeogram_example.pdf}
...
and genomic positions in each genomic category. The range of each category will be inferred from the minimun position
...
\begin{figure}[h!]
\centering
\includegraphics{figures/genomicInitialize_TP.pdf}
...
calculate range of y-values to arrange data points. Users can either specify the index of numeric columns in {\tt data} by {\tt numeric.column}
or set {\tt ylim}. If none of them are set, the function will try to look for all numeric columns
in {\tt data} (of cource, excluding the first three columns), and set them as {\tt numeric.column}.
...
expects a data frame containing regions and a data frame containing values. The data column for 
...
to add {\tt ...} as the final argument in {\tt circos.genomicPoints} to pass such informatioin into it.
...
{\tt circos.genomicLines} is similar as {\tt circos.lines}. The setting of graphical paramters
...
or a seperated vector {\tt y}. The labels of text can be specified either by {\tt labels.column}
...
{\tt ylim} is re-defined inside the function. The y-axis will be splitted into several part
with equal height and graphics will be drawn on each 'horizontal' lines (y = 1, 2, ...).

When input data is a single data frame containing one or more numeric columns, 
...
All low-level genomic graphical functions will draw on the 'horizontal line' {\tt y = i} in which {\tt i} is the index of current numeric column 
and the value of {\tt i} can be obtained by `getI`.
...
genomic category, and {\tt value} contains columns in current data frame excluding the first three columns.
...
Please see figure \ref{fig:genomicPoints}, figure \ref{fig:genomicLines} and figure \ref{fig:genomicRect}
for examples of using different settings in {\tt circos.genomicTrackPlotRegion}.

\begin{figure}[h!]
\centering
\includegraphics{figures/genomic_points.pdf}
\caption{Plotting points in differnet ways. Plotting points from A) a data frame with one numeric column;
...
four numeric column and under {\tt stack} mode.}
\label{fig:genomicPoints}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics{figures/genomic_lines.pdf}
\caption{Plotting lines in differnet ways. Plotting lines from A) a data frame with one numeric column;
...
is set to {\tt segment}.}
\label{fig:genomicLines}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics{figures/genomic_rect.pdf}
\caption{Plotting rectangles in differnet ways. Plotting lines from A) a data frame with four numeric column and under {\tt stack} mode.
B) a list of two data frames under {\tt stack} mode; C) and D) adding rectangles with self-defined {\tt panel.fun}.}
\label{fig:genomicRect}
\end{figure}
...
For example, some horizontal lines and texts need to be added to each cell and axis is needed to put on top of each cell:
...
\begin{figure}[h!]
\centering
\includegraphics{figures/genomic_link.pdf}
\caption{Add links from two set of regions.}
\label{fig:genomicLink}
\end{figure}

\subsection{Highlight chromosomes}

{\tt highlight.chromosome} provides a simple to highlight chromosomes. Just remember
to use transparent filled colors. The position of the highlighted region
and be fine-tuned by {\tt padding} argument. (figure \ref{fig:highlight})
...
\begin{figure}[h!]
\centering
\includegraphics{figures/genomic_highlight.pdf}
...
We specifically implement several high-level functions which may help to visualize genomic data.
...
a position transformation function {\tt posTransform.default} which distribute positions uniformly in current chromosome.
...
\begin{figure}[h!]
\centering
\includegraphics{figures/position_transformation_line.pdf}
...
{\tt circos.genomicDensity} calculate how much a genomic window is covered by {\tt region}

<<echo=TRUE, eval=FALSE>>=
circos.genomicDensity(bed)
circos.genomicDensity(bed, window.size = 1e6)
circos.genomicDensity(bed_list)
@
...
\begin{figure}[h!]
\centering
\includegraphics{figures/rainfall_plot.pdf}
