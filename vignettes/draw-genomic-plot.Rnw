...
\begin{itemize}
  \item {\tt circos.genomicTrackPlotRegion}: create a new track.
  \item {\tt circos.genomicPoints}: add points
  \item {\tt circos.genomicLines}: add lines
  \item {\tt circos.genomicRect}: add rectangles
  \item {\tt circos.genomicText}: add text
  \item {\tt circos.genomicLink}: add links
\end{itemize}

The genomic functions are implemented by basic circos functions (e.g. {\it circos.points}),
thus, you can customize your own plots by both genomic functions and basic circos functions.

\section{Input data type}

{\it circlize} expects input data as a data frame or a list of data frames in which there are at least three columns in it.
...
Normally, the genomic categories is measured by chromosomes. The easiest way is to used
