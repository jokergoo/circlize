...
By the way, if you don't want to draw ideogram, reading the cytoband file is also useful because it tells you the
range of the choromosome and help you to allocate chromosomes in the circle.
...
and we do not need the line to be too thick because genomic graphic is huge.
...
initialization step. (But to be rigorous, order of {\tt factors} matters if graphics arguments like {\tt bg.border}
or {\tt bg.col} is a vector with length larger than 1. However, in most circumstance, we do not need to set {\tt bg.border} or
{\tt bg.col} as a vector, so you don't need to worry about it and just ignore this note).
...
For other tracks of genomic graphics, the genomic coordiate (positions on chromosomes) are x-values and measurements on
...
circular layout is really subjective, such as the position and colors of the ideogram, 
...
to draw ideogram. Thus users can draw their style of ideogram according the above example codes. 
With such low level circos graphics functions provided in {\it circlize}, there would be no difficulty for users to implement their own genomic graphics.
All you need to remember is that {\bf complicated graphics are assembled by simple graphics}.


Finally, more informative and specialized genomic graphics are figure \ref{fig:genomic}, figure \ref{fig:twochr} and figure \ref{fig:zoom}. Figure
\ref{fig:twochr} in fact combines two independent circos plots, and figure \ref{fig:zoom} re-defined sector width to put two zoomed chromosomes
in half part of the circle. Users can refer to the main vignette to find out how to implement.
...
@
\end{center}
\caption{Detailed genomic graphic}
\label{fig:genomic}
\end{figure}
...
@
\end{center}
\caption{Chromosome 7 and 8 are zoomed in at the half of the circle}
\label{fig:zoom}
\end{figure}
