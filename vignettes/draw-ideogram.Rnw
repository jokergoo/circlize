...
by default {\tt read.table} would store such data as {\tt integer} mode. The sumation of such large integers
...
With such low level circos graphic functions provided in {\it circlize}, there would be no difficulty for users to implement their own genomic graphics.
