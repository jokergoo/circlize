...
In this short vignette, we would show you how to customize the circular style of phylogenetic tree through {\it circlize}
...
The data we are using here is from {\tt ape} package. The {\tt bird.orders} data set is some kind related to species of birds.
We also calcualte some other values such as the label for each bird (which is its name.). We split the tree into six categories by {\tt cutree}
...
track, we draw the name of each bird, with different color to represent different category.
...
executation will stop. You can see the advantage of {\it circlize} that if you replace {\tt circos.lines} to {\tt lines}, then you can almost
...
dendrogram, it would be more flexible to customize your figure such as adding 
