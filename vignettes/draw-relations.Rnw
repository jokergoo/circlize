...
<<label=figcorrelation1, echo=FALSE, eval=FALSE>>=
par(mfrow = c(2, 1))
@

<<label=figcorrelation2, echo=TRUE, eval=FALSE>>=
set.seed(12345)
n = 9
...
<<label=figcorrelation3, echo=TRUE, eval=FALSE>>=
...
<<label=figcorrelation4, echo=TRUE, eval=FALSE>>=
library(circlize)
factors = rownames(m2)
colors = 1:n
...
<<label=figcorrelation5, echo=TRUE, eval=FALSE>>=
rn = rownames(m2)
...
The finnal figure looks like figure \ref{fig:correlation} (top part) and a more pretty
figure is on the bottom part of figure \ref{fig:correlation} in which correlatioins are
measured by continuous colors.

<<label=figcorrelation6, echo=FALSE, eval=FALSE>>=
...
\begin{figure}
\begin{center}
<<label=figcorrelation, fig = TRUE, echo = FALSE, width=4, height=8, results=hide>>=
<<figcorrelation1>>
<<figcorrelation2>>
<<figcorrelation3>>
<<figcorrelation4>>
