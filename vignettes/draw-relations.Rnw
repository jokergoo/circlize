...
The second example is draw a table in a circular layout where rows and columns are different
measurements. E.g. a table in which rows are Countries of Nobel Prize winners and the columns
are the areas.

As usually, we first generate an example data. In the example data, letters in upper case is
one measurement and letters in lower case is another measurement. The number is the amount
...
with rownames and colnames of the matrix.
...
Add draw the circular layout of this table (figure \ref{fig:table}). In the following code, the sectors with their names in
...
between two measurements by {\tt gap.degree} in {\tt circos.par}.
