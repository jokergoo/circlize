...
{\it There is a new and more comprehensive vignette in this package which is focusing 
on plotting genomic figures. Nevertheless, this vignette is still useful for users to 
get a clue on how to draw genomic graphics by very basic low-level graphical functions.}
...
In the following example we draw the ideogram for human.

The cytoband data for human can be downloaded from \\
...
by default {\tt read.table} would store such data as {\tt integer} mode. {\it circlize} will sum up such positions
...
form 1000 Genome project which have not 'chr' prefix, remember to add it.

Before we initialize the circular layout, we need to set some graphic parameters. 
Here we do not need any cell paddings and we do not need lines to be too thick 
because genomic graphic is always huge.
...
for(chr in chromosome) {
    ...
    circos.axis(h = 0.5, major.at = major.at,
		...
		labels.direction = "clock.wise", labels.niceFacing = TRUE,
