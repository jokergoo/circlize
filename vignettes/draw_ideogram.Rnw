...
Before we initialize the circular layout, we need to set some graphic parameters. Here we do not need any cell paddings
...
In the initialization step, width of 
each sector corresponds to the length of each chromosome. Also the order of sectors is
determined in this step. Here we must explicitly
...
of {\tt levels} of variable {\tt factors}, so do not be confused here.
...
Here we will draw ideogram, axis and the chromosome names in one track 
...
a new track and add line and text in the specified cell.
...
The final figure is figure \ref{fig:ideogram}.
