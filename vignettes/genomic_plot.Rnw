...
circos.genomicInitialize = function(...) {
    circos.par(unit.circle.segments = 3500)
...
\begin{enumerate}
    ...
    \item Positions for text are assigned to new variables according to their groups.
    \item Do the position transformation on text and add labels at different {\tt y}.
    \item Add the position transformation line manually. Note the line for lower text and higher text
