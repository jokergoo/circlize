...
Example code for figure \ref{fig:genomicpoints} are as follows: points tracks (top left in figure \ref{fig:genomicpoints}), from track A to track F:
...
For line tracks (top right in figure \ref{fig:genomicpoints}), from track A to track F:
...
For rectangle tracks (bottom left in figure \ref{fig:genomicpoints}), from track A to track D:
...
There is a function {\tt circos.genomicPosTransformLines} which adds a line from untransformed regions
to transformed regions. Note {\tt circos.genomicPosTransformLines} will create a new track.
In the function, {\tt direction} controls whether the position transformation track is inside or outside
the track which is created by {circos.genomicPosTransformLines}. Please see figure \ref{fig:postransform} for examples.
...
@
\end{center}
\caption{Transformation of genomic positions. Top: position transformed track is inside; Bottom: Position transformed track is outside.}
...
text and transform position properly. Since such text position transformation relies on font size of text
and which track the text is in, the usage of {\tt posTransform.text} is a little bit complex. Currently, {\tt posTransform.text}
only makes sense if it is called inside {\tt circos.genomicText} and only works when {\tt facing} is set to
{\tt clockwise} or {\tt reverse.clockwise}.
...
For the next track, position transformation lines are going to be plotted. Now code will be a little complex. Since such text position transformation
relies on text settings (e.g. font size, font family) and track for the text, we need to go back to the track where text
position transformation happens. Such information should be collected when plotting transformation lines.
The solution by {\it circlize} is to save the real call by {\tt quote} and pass it to {\tt posTransform} argument. 
Inside {\tt circos.genomicPosTransformLines}, {\tt posTransform.text} will be called by {\tt panel.fun} by {\tt eval},
...
If the track where text position is transformed is after the track where transformation lines are plotted, things will be more
complicated. Since {\tt circos.genomicPosTransformLines} needs information of the text, but at that moment, 
track which transforms text position has not be created.
The solution is first creating a empty track for position transformation lines, then creating and plotting the position transformation track. 
Finally go back to the transformation line track to add transformation lines afterwards.
...
visualizing text, please refer to figure \ref{fig:postransformtext}.

\setkeys{Gin}{width=\textwidth}
\begin{figure}[h!]
\begin{center}
<<label=figpostransformtext, fig = TRUE, echo = FALSE, width = 6, height = 6, results=hide>>=
source("src/genomic-07-posTransformLinesText.R")
@
\end{center}
\caption{Transformation of text positions}
\label{fig:postransformtext}
\end{figure}
\setkeys{Gin}{width=\textwidth}
