...
position transformation happens. Such information should be collected where plotting transformation lines.
...
If the track where text position is transformed is after the track which plots transformation lines, things will be more
complicated. Since {\tt circos.genomicPosTransformLines} needs information of the text, but at that moment, 
track in which position of text will be transformed has not be created.
The solution is first create a empty track for position transformation lines, then create and plot track in which position of text
is transformed. Finally go back to the transformation line track to add transformation lines afterwards.
