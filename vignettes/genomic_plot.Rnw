...
circos.genomicInitialize = function(...) {
	circos.par(unit.circle.segments = 350)
...
\title{An introduction of drawing genomic figures with \textbf{circlize}}
...
cytoband file is \url{http://hgdownload.cse.ucsc.edu/goldenpath/hg19/database/cytoBand.txt.gz}.
...
download cytoband file from UCSC server automatically (If it exists in UCSC).
...
{\tt chromosome.index}. Please note this argument is only used for subsetting but not for 
ordering (figure \ref{fig:genomic_initialize_ideogram} B).
...
\begin{itemize}
  \item If the first column is not a factor, the order of chromosomes would be 
      {\tt unique(cytoband[[1]])} (figure \ref{fig:genomic_initialize_ideogram} C). 
  \item If {\tt cytoband} is provided as a data frame, and if the first column is a factor, 
      the order of chromosome would be {\tt levels(cytoband[[1]])}  (figure \ref{fig:genomic_initialize_ideogram} D). 
  \item If {\tt sort.chr} is set to {\tt TRUE}, chromosomes will be sorted 
      first by numbers then by letters (figure \ref{fig:genomic_initialize_ideogram} E).
\end{itemize}
...
If {\tt cytoband} is specified as a file path, or {\tt species} is specified, the order 
...
(figures from the first two commands are in figure \ref{fig:genomic_initialize_ideogram} F, G).
...
(\ref{fig:genomic_initialize_ideogram} H, I).
...
The input data for the function is a data frame with at least three columns. The first 
...
In the next example, we plot the transcripts for TP53, TP63 and TP73 in a circular layout (figure \ref{fig:genomic_gene_model}).
...
so same strategy can be applied to zoom chromosomes (figure \ref{genomic_zoom}). Be careful with the order of chromosomes.
...
We can only use {\tt circos.genomicInitialize} to deal with zoomed chromosomes because chromosome 
with name "zoom_chr1" is not a normal chromosome.
...
numeric columns in {\tt data} by {\tt numeric.column} (named index or numeric index)
...
Note here {\tt numeric.column} is measured in {\tt value} which {\tt numeric.column} in
{\tt circos.genomicTrackPlotRegion} is measured in the complete data frame (There is a
difference of 3!). If you use character index instead of numeric index, you do not need
to worry about it. 
...
All low-level genomic graphical functions will be drawn on each 'horizontal line' {\tt y = i} 
in which {\tt i} is the index of current numeric column and the value of {\tt i} can 
...
In this section, we will show examples of making genomic plots under different modes.

\subsection{Points}

To make plots more clear to look at, we only add graphics in the 1/4 of the circle and
initialize the plot with chromosome 1 (figure \ref{fig:genomic_application_points}).
...
Note here {\tt value} is a data frame with only one column.
...
Note each segment is added at corresponding y-values.
...
and be fine-tuned by {\tt padding} argument which are percent of corresponding height 
and width in the highlighted region.

First let's create four tracks with random points
...
If you only want to highlight some regions in a chromosome, use {\tt draw.sector} directly. 

\section{High-level genomic functions}

\textbf{circlize} implements several high-level functions which may help to visualize genomic data.
...
regions. There is already a function called {\tt circos.genomicPosTransformLines} which does the job.
Same value for {\tt posTransform} which is used before should be passed to {\tt circos.genomicPosTransformLines}.
...
the track which is created by {circos.genomicPosTransformLines}, and {\tt horizontalLine} controls
whether add lines to identify the regions.
...
For the next track where position transformation lines are plotted. Code will be a little complex.
Since the calculation of the position of the line ends which corresponds to the transformed track
relies on text settings (e.g. font size, font family) and track for the text (i.e. the font size, family and
the track will affect the position of text), we need to go back to the track where text position transformation happens
and use the same settings for the text. Such information should be collected when plotting transformation lines.
The solution by \textbf{circlize} is to wrap {\tt posTransform.text} with pre-defined parameters,
...
If the track where text position is transformed is added after the track where transformation 
lines are plotted, things will be more complicated. Since {\tt circos.genomicPosTransformLines} 
needs information of the text, but at that moment, track which transforms text positions 
has not be created. The solution is first creating a empty track for position transformation lines, 
then creating and plotting the position transformation track. Finally go back to the 
...
tr_track = get.cell.meta.data("track.index") # track for 
...
For examples and comparison between {\tt posTransform.default} and {\tt posTransform.text} when
...
genomic densities. In the plot, red corresponds to hyper-methylated DMRs and blue corresponds
to hypo-methylated DMRs (figure \ref{fig:genomic_rainfall}).
