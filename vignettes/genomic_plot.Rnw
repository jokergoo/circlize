...
{\it circlize} provides a simple function {\tt generateRandomBed} which can generate 
...
{\tt integer} by default. In initialization of circos plot, {\it circlize} needs to 
...
In {\it circlize}, there already provides a position transformation function {\tt posTransform.default} 
...
There is another position transformation function {\tt posTransform.text} provided in {\it circlize} that can smartly position
...
The solution by {\it circlize} is to wrap {\tt posTransform.text} with pre-defined parameters,
