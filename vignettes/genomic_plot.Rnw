...
\begin{enumerate}
    ...
    by looking at whether the middle points of the corresponding region are in the odd intervals (i.e., the first interval, the third interval, ...)
    or even intervals. Of course, in this step, users should determine their own rule to separate the groups.
    \item Different positions for text are assigned according to their groups.
    \item Do the position transformation on text and add labels correspond to different {\tt y}.
    \item Add the identification line manually. Note the line for lower text and higher text
...
<<genome_more_labels, echo = FALSE, out.width = "\\textwidth", out.height = "0.5\\textwidth", fig.width = 10, fig.height = 5, fig.cap = "Position transformation for a lot of text. A) put text on two layers; B) put text on one layer.">>=
par(mfrow = c(1, 2), mar = c(1, 1, 1, 1))
