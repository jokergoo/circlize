...
download cytoband file from UCSC server automatically (If it exists in UCSCl. As you can guess,
the URL template we use is {\tt http://hgdownload.cse.ucsc.edu/goldenpath/$species/database/cytoBand.txt.gz}).
