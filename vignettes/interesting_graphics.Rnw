...
The two arrows are plotted in the canvas coordinate. An example of a real-time clock is in {\tt Examples} section in
...
different colors (figure \ref{fig:dartboard}). The most inside green ring and red circle are plotted by {\tt draw.sector}.
...
Figure \ref{fig:painting} is a circular style of Keith Haring's great doodle. 
