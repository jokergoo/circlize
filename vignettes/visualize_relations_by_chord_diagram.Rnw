...
relations come from the frst column and end at the second column, the third column represents the strength
of the relation.
...
For adjacency list, it is quite easier, you only need to set these graphical settings
as a vecotr.
...
But sometimes sort links according to the width on one sector is useful to detect the 
features in the plot. {\tt link.sort} and {\tt link.decreasing} can be set to control
...
text(-0.9, 0.9, "A", cex = 1.5)
chordDiagram(mat, link.sort = TRUE, link.decreasing = FALSE)
text(-0.9, 0.9, "B", cex = 1.5)
@

<<chord_diagram_link_order, echo = FALSE, fig.align = "center", out.width = "\\textwidth", out.height = "0.5\\textwidth", fig.width = 7, fig.height = 3.5, fig.cap = "Orders of links. A) set {\\tt self.decreasing} to {\tt TRUE}; B) set {\\tt self.decreasing} to {\tt FALSE}.">>=
par(mfrow = c(1, 2))
...
to columns which -1 means the direction is from columns to rows.
By default, links will have unequal height of root (figure \ref{fig:chord_diagram_directional} A, B, C) 
...
text(-0.9, 0.9, "A", cex = 1.5)
chordDiagram(mat, directional = 1, diffHeight = 0.08)
text(-0.9, 0.9, "B", cex = 1.5)
chordDiagram(mat, directional = -1)
text(-0.9, 0.9, "C", cex = 1.5)
@
...
arr.col = data.frame(c("S1", "S2", "S3"), c("E5", "E6", "E4"), 
    c("black", "black", "black"))
chordDiagram(mat, directional = 1, direction.type = "arrows",
...
(figure \ref{fig:chord_diagram_directional} G)
...
If {\tt diffHeight} is set to a negative value, the start roots are longer than the end roots (figure \ref{fig:chord_diagram_direction} H)
...
How to set self links dependends on specific scenarios. A link have two roots that all correspond to the value.
If it is a self link, the two roots will be on a same sector. If the link is plotted in the normal way,
the value for this link would be counted twice and sometimes it is problematic.
Take ajacency matrix for example, if users think the width of each sector should be exactly identical 
to the sum of rows or columns, the link should be degenerated as a 'mountain' with the width corresponding to the value.
In another scenario, suppose the connection represents the migration from countries, then
...
The {\tt self.link} argument can be set to 1 or 2 for different scenarios. You may see the difference in figure \ref{fig:chord_diagram_self_link}.

<<chord_diagram_self_link1, eval = FALSE, echo = c(1, 3)>>=
mat2 = matrix(runif(16), 4); rownames(mat2) = colnames(mat2) = letters[1:4]
chordDiagram(mat2, self.link = 1)
text(-0.9, 0.9, "A", cex = 1.5)
chordDiagram(mat2, self.link = 2)
text(-0.9, 0.9, "B", cex = 1.5)
@
...
add axes on the grid track. But it is easy to customize with self-defined code.
